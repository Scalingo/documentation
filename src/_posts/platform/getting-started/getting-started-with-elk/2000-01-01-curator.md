---
title: Curator
modified_at: 2024-06-28 12:00:00
...
{% note %}
   ...
   context described on [the Overview page]({% post_url platform/getting-started/getting-started-with-elk/2000-01-01-overview %}).\
...
2. To instruct the Python buildpack to install Curator and its dependencies,
   ...
   git commit --message="Add Curator requirements"
   ```
...
3. To configure the connection to the Elasticsearch database, create a file
   ...
   ```bash
   git add curator.yml
   git commit --message="Add Curator configuration"
   ```
...
4. To configure the database indices lifecycles, create a file named
   ...
   git commit --message="Add Curator policies"
   ```
...
5. Curator is not a daemon, it is designed as a one-off process. To be able
   ...
   [Scheduler](https://doc.scalingo.com/platform/app/task-scheduling/scalingo-scheduler).
   ...
   ```bash
   git add cron.json
   git commit --message="Add Curator cron job"
   ```
...
{% note %}
   * When using Curator with an Elasticsearch® by Scalingo addon, and as long
     as we are [stuck with this constraint]({% post_url platform/getting-started/getting-started-with-elk/2000-01-01-overview %}#planning-your-deployment).
     we strongly recommend to stick with version `7.0.1` of Curator.
...
2. Don't forget to commit the change:
   ...
   ```bash
   git add requirements.txt
   git commit --message="Update Curator to 8.0.15"
   ```
...
Curator can use a [configuration file](https://www.elastic.co/guide/en/elasticsearch/client/curator/current/configfile.html).
This file is mostly used to configure the Elasticsearch® database connection
(URL, credentials, ...), as well as the logging settings. This file is written
in YAML. In our previous examples, it's named `curator.yml`.

Curator also requires an [action file](https://www.elastic.co/guide/en/elasticsearch/client/curator/current/actionfile.html).
This file describes the list of actions Curator will run along with their
options. This file is written in YAML. In our previous examples, it's named
`log-clean.yml`.
