...
**Logstash** is a data processing pipeline, taking any source data as input. It
...
4. Attach the Elasticsearch® by Scalingo addon:
...
4. Create a Sandbox Elasticsearch® by Scalingo addon and attach it to your app:
...
**[Log drains]({% post_url platform/app/2000-01-01-log-drain %})** are a
Scalingo feature that, once activated, automatically sends every log line
generated by an application to an ELK stack.

When using this feature, the application name and container index are passed in
the HTTP query and the message is in the request body. To parse this and create
a meaningful index, you can use the following Logstash configuration (if your
