---
title: Logs
modified_at: 2026-09-23 00:00:00
tags: app logs
index: 18
---
...
To keep the platform stable, live logs ingestion has a few limits:

* Each container can ingest up to **16,384 log lines per minute**.
* Each container can ingest up to **64 MiB per minute**.
* Very long messages may count as multiple lines in this quota.
* When the limit is reached, an overflow line is added once, then extra lines are dropped until the next minute.
...
{% note %}
The archives links are protected by a token with a 1 minute life duration.
If you get this error: `unauthorized, wrong token` at archive download, reload the page to get fresh archive links.
{% endnote %}
