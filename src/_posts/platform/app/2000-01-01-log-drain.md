...
layout: page
title: Log Drains
description: "Configure Scalingo log drains from CLI to forward app and add-on logs to Datadog, New Relic, Better Stack, Papertrail, Elastic, or Syslog, and manage drain lifecycle safely."
...
* [Scalingo hosted Elastic Stack](#scalingo-hosted-elk-stack)
([how to setup]({% link _tutorials/elastic-stack/overview.md %}))
* [AppSignal](#appsignal)
...
{% warning %}
If a log drain target fails to accept 10 consecutive log lines, it is placed in quarantine for 5 minutes. During that time, no log lines are sent to it.

If the target still fails after quarantine is lifted, the quarantine duration increases progressively to 10 minutes, 15 minutes, 20 minutes, and so on.
{% endwarning %}
...
```
Parameters:
* `url`: authentication url to access to the Elastic Stack server.
More information on [Elastic Stack on Scalingo]({% link
_tutorials/elastic-stack/overview.md %})
