...
| Database   | `db_kind`  | Tools                                             |
| ---------- | ---------- | ------------------------------------------------- |
| PostgreSQL | `pgsql`    | `psql`, `pg_basebackup`, `pg_controldata`,
                            `pg_dump`, `pg_isready`, `pg_recvlogical`,
                            `pg_restore`, `pg_test_fsync`, `pg_upgrade`,
                            `pg_archivecleanup`, `pg_config`, `pg_ctl`,
                            `pg_dumpall`, `pg_receivewal`, `pg_resetwal`,
                            `pg_rewind`, `pg_test_timing`, `pg_waldump`       |
| MySQL      | `mysql`    | `mysql`, `mysqldump`                              |
| MongoDB    | `mongo`    | `mongo`, `mongodump`, `mongorestore`,             |
