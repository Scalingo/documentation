...
### Access a Database on Multiple Applications
...
In the application containing the database, you should have an environment variable containing the connection string needed to connect to the database. For a Postgresql addon, it should look like `SCALINGO_POSTGRESQL_URL`. It contains your database address, as well for its credentials. Keep this value secret!

Copy this value, and in the application that needs to access it, you can create a new environment variable, and give it the value you just copied. After a restart, your app will have the connection address it needs.
