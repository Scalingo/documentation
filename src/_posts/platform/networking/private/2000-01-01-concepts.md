...
description: "Learn Private Network core concepts: addressing model, private DNS naming, communication rules, and constraints that shape secure intra-project networking."
...
{% note %}
This networking interface exists only for application containers attached to a
Private Network.\\
Databases and other managed resources are not attached to a Private Network and
are not reachable through this interface.
{% endnote %}
...
interactions between them, they are each assigned a private domain name.

{% note %}
These domain names are scoped to the Private Network.\\
**They resolve to an IP address only usable within the same Private Network**.
{% endnote %}
...
to one of them. This routing process, called **load-balancing**, depends on
the DNS client.
...
DNS records in a Private Network have a Time To Live (TTL) of **5 seconds**.
