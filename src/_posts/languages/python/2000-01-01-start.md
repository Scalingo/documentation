...
`WEB_CONCURRENCY` controls how many application worker processes are started by
servers such as [Gunicorn][gunicorn-settings] or [Uvicorn][uvicorn-settings].
Lowering this value can reduce memory usage when your application uses too much
RAM or starts swapping. If you set a fixed value, review it when you change the
container size.

[gunicorn-settings]: https://docs.gunicorn.org/en/stable/settings.html#workers
