...
databases/mysql/about/2000-01-01-overview %}) or [Scalingo for
PostgreSQL®]({% post_url databases/postgresql/about/2000-01-01-overview %})
database has to be added to your application. This addon will inject the
environment variable `SCALINGO_MYSQL_URL` or `SCALINGO_POSTGRESQL_URL` and
alias it to `DATABASE_URL`. We recommend using the alias.
