...
In the following example we will set a rate limit of `1 request per second per IP`.
...
{% warning %}
  ...
  If you want to apply a rate limit on all endpoints of your application you need to set-up 2 apps
  (1 web app and 1 reverse proxy app) by using our
