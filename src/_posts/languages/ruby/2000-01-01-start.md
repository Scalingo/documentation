...
Each Puma worker is a separate Ruby process. If your application uses too much
memory, tune `WEB_CONCURRENCY` and `MAX_THREADS` using your application metrics.

For new Ruby applications, the buildpack sets `MALLOC_ARENA_MAX=2` by default
to limit glibc memory arenas. On memory-constrained threaded workloads, you can
test `MALLOC_ARENA_MAX=1`, but this may reduce performance:

```bash
$ scalingo --app my-app env-set MALLOC_ARENA_MAX=1
```

If memory fragmentation remains an issue, you can also test the
[jemalloc buildpack][jemalloc-buildpack].


[web_concurrency_file]: https://github.com/Scalingo/ruby-buildpack/blob/master/lib/language_pack/ruby.rb
[jemalloc-buildpack]: {% post_url platform/deployment/buildpacks/2000-01-01-jemalloc %}
