...
As a result, if initializers, for example, or an asynchronous worker start performing operations before the migration has been executed, the application may load the schema `pre-migration`, and so a restart may be necessary after the migration, however short it may be.
