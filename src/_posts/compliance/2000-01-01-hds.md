...
The HDS certification enjoins Scalingo to maintain a list of *Points of contact health professional* for each of its customer.
These points of contact must be able to designate an health professional to Scalingo when required (for example, to access to health data or to manage the relationship with the client, ...)
...
Le rôle de ces points de contact est d'être en mesure de désigner à Scalingo un professionnel de santé lorsque cela est nécessaire (par exemple, pour accéder aux données de santé, pour gérer des relations avec le patient, etc.)
