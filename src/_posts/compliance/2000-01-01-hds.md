...
# 🇬🇧 What is a "contact point" ("point de contact" in French)? {#en-hds-contact-point}

The HDS certification enjoins Scalingo to maintain a list of contact points for each of its customer.
These contact points must be able to designate an health professional to Scalingo when required (for example, to access to health data or to manage the relationship with the client, ...)
Moreover, Scalingo must be able to provide this list without delay, on demand of the competent authorities, particularily if the certification is to be suspended or removed.
...
1. Mise à disposition et maintien en condition opératThe search found this: ionnelle des sites physiques permettant d’héberger l’infrastructure matérielle du système d’information utilisé pour le traitement des données de santé ;
...
## 🇫🇷 Qu'est-ce qu'un "point de contact" ? {#fr-hds-contact-point}

La certification HDS impose à Scalingo de maintenir une liste de **points de contact** pour chacun de ses clients.
Le rôle de ces points de contact est d'être en mesure de désigner à Scalingo un professionnel de santé lorsque cela est nécessaire (par exemple, pour accéder aux données de santé, pour gérer des relations
avec le patient, etc.)
En outre, Scalingo doit être en capacité de transmettre sans délai cette liste à l’autorité compétente sur demande, notamment en cas de suspension ou de retrait de la certification.
