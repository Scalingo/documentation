...
## 🇬🇧 Is the pricing different for HDS applications compared to non-HDS applications? {#en-billing}

No, HDS applications do not incur any additional cost for equivalent resources compared to non-HDS applications. However, they are subject to additional constraints, [detailed in the section below](#en-apps-addons-restrictions).

## 🇬🇧 What restrictions apply to HDS applications and addons? {#en-apps-addons-restrictions}

HDS applications must comply with the following requirements:

- Have at least 2 `web` containers to ensure high availability.
- All databases of an HDS application must use a [Business plan]({% post_url databases/about/2000-01-01-overview %}#business).
- MongoDB and InfluxDB databases are not available for HDS applications.
...
## 🇫🇷 La tarification est-elle différente pour des applications HDS par rapport à des applications non-HDS ? {#fr-billing}

Non, les applications HDS n’entraînent aucun surcoût à ressources équivalentes par rapport à des applications non-HDS. Elles sont cependant soumises à des contraintes supplémentaires, [détaillées dans la section ci-dessous](#fr-apps-addons-restrictions).

## 🇫🇷 Quelles restrictions s’appliquent aux applications et addons HDS ? {#fr-apps-addons-restrictions}

Les applications HDS doivent obligatoirement respecter les conditions suivantes :

- Disposer d’au minimum 2 conteneurs `web` afin d’assurer la haute disponibilité.
- Toutes les bases de données d’une application HDS doivent utiliser un [plan Business]({% post_url databases/about/2000-01-01-overview %}#business).
- Les bases de données MongoDB et InfluxDB ne sont pas disponibles pour les applications HDS.
