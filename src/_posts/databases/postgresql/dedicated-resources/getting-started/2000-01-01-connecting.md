...
Scalingo, and `env-get SCALINGO_POSTGRESQL_URL` expose the default **Public
RW** endpoint.

To use the **Private Peering RW** endpoint, for example through Outscale Net
Peering, replace the hostname and port in the connection URI with the hostname
and port of this endpoint.
