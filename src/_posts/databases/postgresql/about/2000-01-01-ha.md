...
availability mechanism. Consequently, we don't consider them to be 
*highly available*. This translates into a lower 
[Service Level Agreement][databases-sc].


## Highly Available plans

Business and Enterprise plans are designed for higher service continuity, 
including during maintenance and platform incidents.

The following schema describes the cluster we setup and maintain for each 
Business plan with two gateways and two database nodes:

{% assign img_url = "https://cdn.scalingo.com/documentation/documentation_postgresql_cluster.png" %}
{% include mdl_img.html %}

{% note %}
On Dedicated Resources, each node–gateway pair is distributed across up to three 
availability zones, providing stronger fault tolerance and higher availability.
{% endnote %}
...
Business and Enterprise plans include one primary node and one or more standby 
nodes. Standby nodes improve service continuity by:

- Maintaining an up-to-date secondary copy of the data.
- Reducing the potential data loss window in disaster scenarios.
- Enabling faster recovery through controlled failover, since standby nodes are 
  already running and synchronised.
