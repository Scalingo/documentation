...
Even though the default user has enough rights to drop a database, this should NOT be done. The default user do not have the rights to re-create the dropped database. In case you want to drop a database, remove it from your application and provision a new one.
