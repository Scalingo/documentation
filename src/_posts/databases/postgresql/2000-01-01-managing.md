...
2. From the command line, list the addons attached to your app:
   ...
   |   ADDON    |                   ID                    |          PLAN           | STATUS  |
   +------------+-----------------------------------------+-------------------------+---------+
   | PostgreSQL | ad-50d873b0-7760-4a4b-8d1a-760ec997b481 | postgresql-starter-1024 | running |
   +------------+-----------------------------------------+-------------------------+---------+
   ```
...
2. From the command line, list the addons attached to your app:
   ...
   +------------+-----------------------------------------+-------------------------+---------+
   ```
3. Locate the `ID` of the addon for which you want to update a user password
4. Update the user password with the following command:
   ```bash
   scalingo --app my-app --addon <addon_id> database-users-update-password <username>
   ```  
5. An interactive prompt asks you for the new password you want to attribute to your user
