...
- Duration before trigger: send the alert when metric value is above the limit
  for the specified duration. Duration is expressed in nanoseconds. Granularity
  of this parameter is the minute. Specifying 4 minutes means that the alert can
  trigger if the value is above the threshold during 3:01 minutes to 4:59
  minutes.
