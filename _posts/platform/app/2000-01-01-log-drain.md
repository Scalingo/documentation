...
To add a log drain to an addon you have to pass its uuid. First you have to list
them with the command to find addon's uuid:
```bash
...
Currently, only databases addons are able to forward logs to a drain.
...
You can also list drains from both application and connected addons with the
option `--with-addons`.
...
connected addons. To remove a drain from only the application you can use
`--only-app` option.
{% endnote %}
