...
If you're DNS configuration is not valid or if we fail to generate a
certificate the platform will automatically retry it later. The time between those tries
is determined using an exponential backoff algorithm (we will wait 1mn for the
first retry, 2 for the second, 4 for the third and so on). This algorithm is
capped at a minimum of 1 try per day.
