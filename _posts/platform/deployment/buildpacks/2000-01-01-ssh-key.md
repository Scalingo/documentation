...
Because the buildpack cannot input a password, the provided SSH key **must** be provided unencrypted. Thus our advice to fetch private GitHub/GitLab repository is to setup a Deploy Key, unencrypted and read-only for target repositories you want to fetch.
