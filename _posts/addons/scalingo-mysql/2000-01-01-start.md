...
If you wish to manually backup your database, please follow [How to dump and restore my MySQL database on Scalingo]({% post_url addons/scalingo-mysql/2000-01-01-dump-restore %}) guide.

## phpMyAdmin

A phpMyAdmin instance is available at
[https://phpmyadmin.scalingo.io](https://phpmyadmin.scalingo.io). The server,
username and password fields must be filled with the information in your
`SCALINGO_MYSQL_URL` environment variable. Note that the *server* input should be in the form *host:port*.
