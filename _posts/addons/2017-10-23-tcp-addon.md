...
When you push a new version our restart you `tcp` containers, the new tcp
containers are started and the platform wait for these containers to
be up and running before switching the traffic to the new ones.

The server is responsible for closing every opened connections with his
clients by catching the `SIGTERM` signal see [container-management]({% post_url
internals/2014-11-25-container-management %}) for more informations. If the
server does not close the connections, the new rules will be updated and next
packets will be dropped causing the client to timeout.
...
to be replaced by our front server internal IP, this mean that a `tcp`
container wont be able to access the real client IP.
