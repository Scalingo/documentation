...
are restarted, newly created `tcp` containers are started and the platform waits
for them to be up and running. Once they are ready, newly incoming TCP
connections will be sent to them and old container will receive a `SIGTERM`
signal, notifying them to stop, see [container-management]({% post_url
