...
To configure composer to export some env keys into files and run `artisan passport:keys` at each deployment, update the `script` element of your composer.json to integrate the values (`post-install-cmd`, `post-create-project-cmd`) below:
