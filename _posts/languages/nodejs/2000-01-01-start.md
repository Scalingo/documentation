...
Your private module should be fetched through the SCM service through SSH, you can specify it this way:

```
"package-name": "git+ssh://git@<SCMhostname>/<workspace>/<repo>.git#master"
```

To authenticate to the SCM service, you need to use our [SSH Private Key Buildpack]({% post_url /platform/deployment/buildpacks/ssh-key %}), this buildpack will let you setup a private SSH key securely in the build environment.
